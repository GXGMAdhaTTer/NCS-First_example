...
def preprocess_image(source_image):
    resized_image = cv2.resize(
        source_image, (NETWORK_IMAGE_WIDTH, NETWORK_IMAGE_HEIGHT))
...
def run_inference(image_to_classify, ssd_mobilenet_graph):
    ...
    for box_index in range(num_valid_boxes):
        ...
        x2 = min(int(output[base_index + 5] *
                     image_to_classify.shape[0]), image_to_classify.shape[0]-1)
        y2 = min((output[base_index + 6] * image_to_classify.shape[1]),
                 image_to_classify.shape[1]-1)
